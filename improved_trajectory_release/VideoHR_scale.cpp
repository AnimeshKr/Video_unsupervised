...
string makedir(char *sdir)
{
    ...
    string nowdir = "/home/xiaolonw/videos/data/pairs/collect_scale/";
    int flen = front.size();
...
int main( int argc, char** argv )
{
	...
	{
		cropcnt ++;
		std::random_shuffle(ratios.begin(), ratios.end(), myrandom);
		float  nowratio = ratios[0];
		//debug 
		printf("%f\n", nowratio);
		...
		int nowhin = y2 - y1;
		int nowwin = x2 - x1;
		if (nowratio < 1) nowhin /= nowratio;
		else nowwin *= nowratio;
		x1 = midxin - nowwin / 2;
		x2 = midxin + nowwin / 2;
		y1 = midyin - nowhin / 2;
		y2 = midyin + nowhin / 2;

		int midxout = (bx1 + bx2) / 2;
		int midyout = (by1 + by2) / 2;
		int nowhout = by2 - by1;
		int nowwout = bx2 - bx1;
		if (nowratio < 1) nowhout /= nowratio;
		else nowwout *= nowratio;
		bx1 = midxout - nowwout / 2;
		bx2 = midxout + nowwout / 2;
		by1 = midyout - nowhout / 2;
		by2 = midyout + nowhout / 2;
