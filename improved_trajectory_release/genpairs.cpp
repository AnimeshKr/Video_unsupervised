...
//	system(cmdenv1.c_str());
	...
	while ((dirp = readdir( dp )))
	{
		...
		while ((dirpv = readdir( dpv )))
		{
			...
			if (tempd == NULL)
			{
				mkdir(pairdir.c_str(), 0777);
			}
			else
				closedir(tempd);

			string imgpairdir = pairdir + "/imgs";
			tempd = opendir(imgpairdir.c_str());
			if (tempd == NULL)
			{
				mkdir(imgpairdir.c_str(), 0777);
			}
			else
				closedir(tempd);


			string cmd1 = "./release/DenseTrackStab " + videoname + " " + pairdir;
			system(cmd1.c_str());
			string cmd2 = "./calbbox " + pairdir;
			system(cmd2.c_str());
			string cmd3 = "./release/Video " + videoname + " " + pairdir;
			system(cmd3.c_str());
			printf("%s\n", pairdir.c_str());
			string cmd4 = "rm -rf " + pairdir + "/traj.txt";
			system(cmd4.c_str());
		}
