...
char filename[1010];
//char *saveDir = "/home/dragon123/IDT/improved_trajectory_release/saveimgs/bike2";
int resizeLen = 150;
...
//	fprintf(stderr, "video size, length: %d, width: %d, height: %d\n", seqInfo.length, seqInfo.width, seqInfo.height);
	...
	int resizeW, resizeH;
	sprintf(filename, "%s/traj.txt",saveDir);
	...
	while(true) {
		...
		//  wxl test
		//if(frame_num == 1000) break;
		int maxLen = max(frame.cols, frame.rows);
		float resizeScale =  resizeLen * 1.0 / maxLen;
		resizeW = frame.cols * resizeScale;
		resizeH = frame.rows * resizeScale;

		Mat frame2;
		cv::resize(frame, frame2, cv::Size(resizeW, resizeH));
		frame =frame2;
		...
		if(frame_num == start_frame) {
			...
			for(int iScale = 0; iScale < scale_num; iScale++) {
				...
				// wxl test
				//sprintf(filename, "%s/%d.txt", saveDir, frame_num);
				//FILE * file = fopen(filename, "w");
				fprintf(file, "%d\n", points.size());
				for(i = 0; i < points.size(); i++)
				{
		...
		for(int iScale = 0; iScale < scale_num; iScale++) {
			...
			for (std::list<Track>::iterator iTrack = tracks.begin(); iTrack != tracks.end();) {
				...
				//int xx = point.x;
				//int yy = point.y;
				pxy[0].push_back(point.x);
				pxy[1].push_back(point.y);
				dxy[0].push_back(iTrack->disp[index].x);
				dxy[1].push_back(iTrack->disp[index].y);
				...
				if(iTrack->index >= trackInfo.length) {
					...
					if(IsValid(trajectory, mean_x, mean_y, var_x, var_y, length) && IsCameraMotion(displacement)) {
						...
						//printf("iTrack->index >= trackInfo.length\n");
						/*
			...
			// wxl test
			fprintf(file, "%d\n", pxy[0].size());
			for(int iout = 0; iout < pxy[0].size(); iout++)
			{
				fprintf(file, "%d %d %f %f\n", pxy[0][iout], pxy[1][iout], dxy[0][iout], dxy[1][iout] );
			}
	...
	sprintf(filename, "%s/wh.txt", saveDir);
	FILE * wh = fopen(filename, "w");
	fprintf(wh, "0 %d %d %d\n", frame_num, resizeW, resizeH);
	fclose(wh);
