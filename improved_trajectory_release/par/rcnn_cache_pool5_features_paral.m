function rcnn_cache_pool5_features_paral(imdb, options, varargin)
...
rcnn_model = rcnn_load_model(rcnn_model,0);
...
for i = opts.start:opts.end
  ...
  save_lock = [opts.output_dir image_ids{i} '.lock'];
  
  if fileExists(save_file) || ~makeDirOrFail(save_lock)
    continue;
  end
  count = count + 1;
  ...
  if options.now + toc(start) > options.timelimit
      error('Time limit!');
  end
